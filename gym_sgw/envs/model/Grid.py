...
from gym_sgw.envs.enums.Enums import MapObjects, Terrains, Actions, Orientations, MapProfiles, MapColors, StateOfNature, Anarchy, Normal


class Grid:

    def __init__(self, map_file: str = None, rows=25, cols=25, random_profile: MapProfiles = MapProfiles.custom_made, mode=1):
        ...
        self.injured_saved = 0
        self.injured_squished = 0
    ...
    def _random_fill_setup(self, grid):
        ...
        if mode == MapProfiles.trolley:
            ...
            p_zombie = 96
            p_battery = 100
        elif mode == MapProfiles.custom_made:  # Default to the uniform case
            p_wall = 30
            p_floor = 60
            p_hospital = 63
            p_fire = 66
    ...
    def _get_score_of_action(self):
        ...
        modes = [Normal, Anarchy, StateOfNature]
        rewards = modes[self.mode-1]
        ...
        """
        the data we will be collecting is:
        1. zombies squashed
        2. peds squashed
        3. injured saved (in hospital)
        4. injured squashed
        """
    ...
    def _get_energy_of_action(self):
        ...
        MUD_DRAIN = -2  # Mud = 2 energy penalty
        FIRE_DRAIN = -5 # Fire = 5 energy penalty
        ...
        # Drain energy if you hit fire (do not remove it from the board)
        if end_cell.terrain == Terrains.fire:
            t_energy += FIRE_DRAIN  # it all began when the fire nation attacked :pensive:
